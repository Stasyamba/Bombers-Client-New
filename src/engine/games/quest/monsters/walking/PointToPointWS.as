    ...
    private function getNewDirection():Direction {
        if (nextPoint.x == _lastPoint.x) {
            if (nextPoint.y > _lastPoint.y)
                ...
                throw Context.Exception("Error in file PointToPointWS.as: can't perform PTP between to identical points")
        } else if (nextPoint.y == _lastPoint.y) {
            if (nextPoint.x > _lastPoint.x)
            ...
            throw Context.Exception("Error in file PointToPointWS.as: PTP can't be performed between point " + _lastPoint.x + ";" + _lastPoint.y + " and " + nextPoint.x + ";" + nextPoint.y)
    }
}
}
